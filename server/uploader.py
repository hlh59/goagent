...
google_iplist = '203.208.46.22|203.208.37.22|203.208.39.22|74.125.71.36|74.125.71.99|74.125.71.104|209.85.229.18|66.102.11.83|66.102.13.17|66.102.13.19|74.125.225.22|74.125.226.119|74.125.226.183|74.125.227.22'.split('|')
_socket_getaddrinfo = socket.getaddrinfo
def socket_getaddrinfo(host, port, *args):
    if 'HTTP_PROXY' not in os.environ and host.endswith('.google.com'):
        conn = RandomTCPConnection(google_iplist, port, 5, 6)
        return [(2, 1, 0, '', conn.socket.getpeername())]
    else:
        return _socket_getaddrinfo(host, *args)
...
def main():
    if len(sys.argv) == 2 and sys.argv[1] != "update" and sys.argv[1] != "rollback":
        ...
        return
    secure = 'HTTP_PROXY' in os.environ
